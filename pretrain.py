...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_step == 0:
            model.eval()
            ckp = f'{args.save_dir}/pretrain-{args.epochs}-{args.batch_size}-{args.accumulation_steps}.pth'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wandb_project', type = str, default = "XiaoXin-Pretrain")
    ...
    parser.add_argument('--data_path', type = str, default = "pretrain.jsonl")
    # 解析命令行参数
    ...
    # 设置 wandb 运行名称
    args.wandb_run_name = f"XiaoXin-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
