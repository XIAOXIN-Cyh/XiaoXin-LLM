...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_step == 0:
            model.eval()
            ckp = f'{args.save_dir}/SFT-{args.epochs}-{args.batch_size}-{args.accumulation_steps}.pth'
...
def init_model(llm_config):
    ...
    ckp = './results/pretrain-1-84-2.pth'
...
if __name__ == '__main__':
    ...
    # 设置 wandb 运行名称
    args.wandb_run_name = f'XiaoXin-SFT-{args.epochs}-{args.accumulation_steps}-{args.batch_size}-{args.learning_rate}'
